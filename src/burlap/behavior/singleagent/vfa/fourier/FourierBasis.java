...
/**
 * An implementation of Fourier Basis functions [1]. This class expects a normalized state variable/feature vector of
 * input states, if it is not normalized, behavior is not well defined. Therefore consider using the
 * {@link ConcatenatedObjectFeatureVectorGenerator} generator with the normalization flag set to convert the OO-MDP
 * {@link State} objects into the necessary input vector.
 * <p/>
 * The higher order the basis functions, the higher the VFA resolution is. Typically, order n will produce (n+1)^d state
 * basis functions (and a copy for each action), where d is the number of state variables. Since this grows quickly,
 * a way to manage the complexity is to simplify the number of coefficient vectors. That is, each basis function is a
 * function of the dot product of the input state variable vector and a coefficient vector {0...n}^d and normally all
 * possible coefficient vectors (and their corresponding basis functions) for order n are produced. However, this class
 * can be told to limit the permitted coefficient vectors to those that have no more than k non-zero entries in the
 * coefficient vector. When k = 1, all features are treated as independently resulting in n*d basis functions.
 * <p/>
 * When using a learning algorithm like {@link GradientDescentSarsaLam} with Fourier basis functions, it is typically a
 * good idea to use the {@link FourierBasisLearningRateWrapper}, which scales the normal learning rate by the inverse of
 * the norm of a basis function's coefficient vector.
 * <p/>
 * <p/>
 * 1. G.D. Konidaris, S. Osentoski and P.S. Thomas. Value Function Approximation in Reinforcement Learning using the
 * Fourier Basis. In Proceedings of the Twenty-Fifth Conference on Artificial Intelligence, pages 380-385, August 2011.
 *
...
public class FourierBasis implements FeatureDatabase {
    ...
    /**
     * Initializes. The coefficient vectors used by this Fourier Basis function will be generated lazily when the first
     * features for an input state/state-action pair are queried. The maximum number of non-zero coefficient entries in
     * a coefficient vector will be set to the maximum (the state variable dimensionality).
     *
    ...
    /**
     * Initializes. The coefficient vectors used by this Fourier Basis function will be generated lazily when the first
     * features for an input state/state-action pair are queried.
     * Setting maxNonZeroCoefficients to one results in treating each state variable as indepdent and thereby producing
     * order*d basis functions (for each action) where d is the number of state variables. Larger values of
     * maxNonZeroCoefficients will result in more variable dependency combinations.
     *
     * @param featureVectorGenerator the state feature vector generator that turns OO-MDP {@link State} objects into double arrays
     * @param order                  the fourier basis order
     * @param maxNonZeroCoefficients the maximum number of entries in coeffient vectors that can have non-zero values.
     */
    public FourierBasis(StateToFeatureVectorGenerator featureVectorGenerator, int order, int maxNonZeroCoefficients) {
        this.featureVectorGenerator = featureVectorGenerator;
        this.order = order;
    ...
    /**
     * Forces the set of coefficient vectors (and thereby Fourier basis functions) used. Use this method only if you
     * want to fine tune the basis functions used.
     *
     ...
     * index may be greater than the number of coefficient vectors because it may refer to an state-action feature's
     * basis function (and there is a copy of each basis function for each action). The basis function used will be the
     * one associated with the coefficient vector at index basisFunction % m, where m is the number of this object's
     * coefficient vectors.
     *
